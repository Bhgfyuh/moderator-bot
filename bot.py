...
async def ban_h(message: Message, command: CommandObject):
    ...
    try:
        if t == "0":
            ...
            await message.answer(f"💀 **БАН НАВСЕГДА:** {message.reply_to_message.from_user.first_name}\n📝 **Причина:** {r}")
        else:
            until = datetime.now() + parse_time(t, 1)
...
async def beer_cmd(message: Message):
    await message.answer(f"🍺 {message.from_user.first_name} жахнул {round(random.uniform(0.5, 5.0), 1)}л пива!")

@dp.message(Command("army"))
async def army_h(message: Message):
    if not is_admin(message.from_user.id): return
    if message.chat.id in chat_members:
