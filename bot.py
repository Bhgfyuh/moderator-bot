...
async def help_cb(call: CallbackQuery):
    ...
    if s == "main":
        ...
        await call.message.edit_text("🛡 **МОДЕРАЦИЯ:**\n\n/mute [время] [причина]\n/unmute\n/warn [причина]\n/unwarn\n/ban [время] [причина]\n/unban [ID]\n/army — сбор", reply_markup=back)
    elif s == "team":
        await call.message.edit_text("🪖 **СТАРШИЙ СОСТАВ:**\n👑 Никита, 🎖 Арлан, 🥈 Ярик, 💻 Олег", reply_markup=back)
    await call.answer()
...
async def mute_cmd(message: Message, command: CommandObject):
    ...
    args = command.args.split(maxsplit=1) if command.args else ["1h", "Не указана"]
...
async def ban_cmd(message: Message, command: CommandObject):
    if not is_admin(message.from_user.id) or not message.reply_to_message: return
    args = command.args.split(maxsplit=1) if command.args else ["0", "Не указана"]
    ...
    await message.answer(f"💀 Бан {message.reply_to_message.from_user.first_name} ({args[0] if args[0]!='0' else 'навсегда'})")
...
async def warn_cmd(message: Message):
    ...
    if warns[uid] >= 3:
        ...
        await message.answer(f"⚠️ Варн {message.reply_to_message.from_user.first_name} ({warns[uid]}/3)")
